...
if [ -f "$TOMCAT_BIN/shutdown.sh" ]; then
    ...
    if ps aux | grep -v grep | grep tomcat10 > /dev/null; then
        ...
        if ps aux | grep -v grep | grep tomcat10 > /dev/null; then
            echo "Tomcat 10 toujours en cours, arrêt forcé..."
            pkill -f tomcat10
            sleep 1
        fi
        echo "Tomcat 10 arrêté"
    else
        echo "ℹ Tomcat 10 n'est pas en cours d'exécution"
    fi
else
    echo "Script shutdown.sh non trouvé, arrêt via systemd ou kill"
...
if [ -f "$TOMCAT_BIN/startup.sh" ]; then
    ...
    if [ $? -ne 0 ]; then
        ...
        sudo systemctl start tomcat10 2>/dev/null || echo "Démarrage manuel nécessaire"
    else
        echo "Tomcat 10 démarré avec succès"
    fi
else
    echo "Script startup.sh non trouvé, démarrage via systemd"
...
if [ -d "$TOMCAT_WEBAPPS/$APP_NAME" ]; then
    ...
    echo "Le répertoire de déploiement n'existe pas encore, attendez quelques secondes"
fi
